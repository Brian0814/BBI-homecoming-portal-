...
export function resolveMailMergeTokens(template: string, attendee: HistoryEntry): string {
  ...
  if (attendee.formData.addDetroitJacket) {
    jacketDetails = `Custom Chapter Jacket (Size ${attendee.formData.jacketSize || "N/A"}, Line Name: "${attendee.formData.jacketLineName || "N/A"}", Entire Line: "${attendee.formData.jacketEntireLineName || "N/A"}", Line #: ${attendee.formData.jacketLineNumber || "N/A"}, Year: ${attendee.formData.jacketCrossingYear || "N/A"})`;
  }
  ...
  return sanitizeEmailText(resolved);
}

export const replaceMergeTokens = resolveMailMergeTokens;

/**
 * Strict sanitizer ensuring the word "Detroit" is completely removed
 * and prevented from appearing in any emails sent or generated by the system.
 */
export function sanitizeEmailText(text: string): string {
  if (!text) return "";
  return text
    .replace(/\bDetroit\s+Homecoming\b/gi, "Homecoming")
    .replace(/\bDetroit\s+Jacket\b/gi, "Chapter Jacket")
    .replace(/welcoming\s+you\s+home\s+to\s+Detroit!/gi, "welcoming you home!")
    .replace(/welcoming\s+you\s+home\s+to\s+Detroit\./gi, "welcoming you home.")
    .replace(/welcoming\s+you\s+home\s+to\s+Detroit/gi, "welcoming you home")
    .replace(/to\s+Detroit!/gi, "!")
    .replace(/to\s+Detroit\./gi, ".")
    .replace(/in\s+Detroit!/gi, "!")
    .replace(/in\s+Detroit\./gi, ".")
    .replace(/in\s+Detroit\b/gi, "")
    .replace(/\bDetroit\b/gi, "")
    .replace(/[ \t]{2,}/g, " ")
    .replace(/ \n/g, "\n");
}
